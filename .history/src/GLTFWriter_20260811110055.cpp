...
// Load wall texture image
tinygltf::Image wallImage;
std::string wallErr;
std::string wallWarn;

if (!gltf.LoadImageData(&wallImage, &wallErr, &wallWarn,
                        "textures/wall.jpg", 0, 0, nullptr))
{
    std::cerr << "Failed to load textures/wall.jpg" << std::endl;
}
model.images.push_back(wallImage);

// Create texture from the image
tinygltf::Texture wallTexture;
wallTexture.source = 0;
model.textures.push_back(wallTexture);

// Material 0 = wall texture
tinygltf::Material wallMaterial;
wallMaterial.pbrMetallicRoughness.baseColorTexture.index = 0;
wallMaterial.pbrMetallicRoughness.metallicFactor = 0.0;
wallMaterial.pbrMetallicRoughness.roughnessFactor = 1.0;
model.materials.push_back(wallMaterial);

// Material 1 = plain blue roof
tinygltf::Material roofMaterial;
roofMaterial.pbrMetallicRoughness.baseColorFactor = {
    0.45, 0.70, 1.00, 1.00
}; // light blue
roofMaterial.pbrMetallicRoughness.metallicFactor = 0.0;
roofMaterial.pbrMetallicRoughness.roughnessFactor = 1.0;
model.materials.push_back(roofMaterial);
...
// Index buffer view
// Wall index buffer view
tinygltf::BufferView wallIndexView;
wallIndexView.buffer = 0;
wallIndexView.byteOffset = (int)(positionBytes + texcoordBytes);
wallIndexView.byteLength = (int)wallIndexBytes;
wallIndexView.target = TINYGLTF_TARGET_ELEMENT_ARRAY_BUFFER;
model.bufferViews.push_back(wallIndexView);

// Roof index buffer view
tinygltf::BufferView roofIndexView;
roofIndexView.buffer = 0;
roofIndexView.byteOffset = (int)(positionBytes + texcoordBytes + wallIndexBytes);
roofIndexView.byteLength = (int)roofIndexBytes;
roofIndexView.target = TINYGLTF_TARGET_ELEMENT_ARRAY_BUFFER;
model.bufferViews.push_back(roofIndexView);
...
model.accessors.push_back(texcoordAccessor);
    ...
    // Wall index accessor
tinygltf::Accessor wallIndexAccessor;
wallIndexAccessor.bufferView = 2;
wallIndexAccessor.byteOffset = 0;
wallIndexAccessor.componentType = TINYGLTF_COMPONENT_TYPE_UNSIGNED_INT;
wallIndexAccessor.count = (int)wallIndices.size();
wallIndexAccessor.type = TINYGLTF_TYPE_SCALAR;
wallIndexAccessor.normalized = false;
model.accessors.push_back(wallIndexAccessor);

// Roof index accessor
tinygltf::Accessor roofIndexAccessor;
roofIndexAccessor.bufferView = 3;
roofIndexAccessor.byteOffset = 0;
roofIndexAccessor.componentType = TINYGLTF_COMPONENT_TYPE_UNSIGNED_INT;
roofIndexAccessor.count = (int)roofIndices.size();
roofIndexAccessor.type = TINYGLTF_TYPE_SCALAR;
roofIndexAccessor.normalized = false;
model.accessors.push_back(roofIndexAccessor);



    // Wall primitive
tinygltf::Primitive wallPrimitive;
wallPrimitive.attributes["POSITION"] = 0;
wallPrimitive.attributes["TEXCOORD_0"] = 1;
wallPrimitive.indices = 2;   // wall index accessor
wallPrimitive.mode = TINYGLTF_MODE_TRIANGLES;
wallPrimitive.material = 0;  // wall material

// Roof primitive
tinygltf::Primitive roofPrimitive;
roofPrimitive.attributes["POSITION"] = 0;
roofPrimitive.attributes["TEXCOORD_0"] = 1;
roofPrimitive.indices = 3;   // roof index accessor
roofPrimitive.mode = TINYGLTF_MODE_TRIANGLES;
roofPrimitive.material = 1;

tinygltf::Mesh gltfMesh;
gltfMesh.primitives.push_back(wallPrimitive);
gltfMesh.primitives.push_back(roofPrimitive);
