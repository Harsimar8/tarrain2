...
std::cout << "\nBuildings that contain height or levels tags:\n";

for (const auto& b : buildings)
{
    if (b.height > 0 || b.levels > 0)
    {
        std::cout << "OSM ID: " << b.id
                  << " | height = " << b.height
                  << " | levels = " << b.levels
                  << std::endl;
    }
...
std::cout << "\nBuildings with height tag: " << heightCount << std::endl;
std::cout << "Buildings with levels tag: " << levelCount << std::endl;
...
if (b.id == 1435819526)
{
    std::cout << "\n===== TEST BUILDING =====" << std::endl;
    std::cout << "OSM ID: " << b.id << std::endl;
    std::cout << "Levels: " << b.levels << std::endl;
    std::cout << "Final height: " << h << " m" << std::endl;

    if (!b.nodeIds.empty())
    {
        auto it = nodes.find(b.nodeIds[0]);
        if (it != nodes.end())
        {
            std::cout << std::fixed << std::setprecision(8);
            std::cout << "Latitude:  " << it->second.lat << std::endl;
            std::cout << "Longitude: " << it->second.lon << std::endl;
        }
    }
