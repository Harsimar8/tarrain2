...
void GLBTiler::exportAllTiles(double tileSize, const std::string& outputFolder)
{
    ...
    writeTilesetJson(minTileX, maxTileX, minTileZ, maxTileZ,
                     tileSize, tilesFolder);

    std::cout << "Total tiles exported: " << exported << std::endl;
    std::cout << "Tiles written to: " << tilesFolder << std::endl;
    std::cout << "=====================================\n";
}
...
double halfX = (rootMaxX - rootMinX) * 0.5;
double halfY = (rootMaxY - rootMinY) * 0.5;
double halfZ = (rootMaxZ - rootMinZ) * 0.5;
